...
list_bit = (7,23,39,55,71,87,103,119)
def bit_b2l(x):
    temp = x/8
    return (list_bit[temp] - x)
...
class Signal(object):
    ...
    def is_msb(self):
        return '1' == self.endian_and_sign[0]
    ...
    def get_signal_code(self):
        ...
        comt = "//Comment:"
        comt += self.comment
        comt += "//< B" + str(self.bit_start) + ":" + str(self.bit_size)
        if self.min_val != 0 or self.max_val != 0:
            comt += "  Min: " + self.min_val_str + " Max: " + self.max_val_str
    ...
    def get_decode_code(self, raw_sig_name, prefix=''):
        ...
        if self.is_msb():
            ...
            while remaining > 0:
                ...
                s = ""
                #s += (" //< remaining:" + str(remaining) + " bits_in_this_byte:" + str(bits_in_this_byte) + "\n")
                s += (
                LINE_BEG + raw_sig_name + " |= ((uint32_t)((bytes[" + str(byte_num) + "] >> " + str(bit_pos % 8 - bits_in_this_byte + 1) + ")")
                s += (" & 0x" + format(2 ** bits_in_this_byte - 1, '02x') + ")) << " + str(remaining - bits_in_this_byte) + ";")
                s += ("    //< " + str(bits_in_this_byte) + " bit(s) from B" + str(bit_pos) + "\n")
...
class Message(object):
    ...
    def get_encode_and_send(self, name):
        ...
        code += ("    uint32_t mid = " + str(self.mid) + ";\n")
        code += ("    uint8_t dlc = " + str(self.dlc) + ";\n")
        code += ("    dbc_encode_" + name + "(bytes, from);\n")
        code += ("    return dbc_app_send_can_msg(mid, dlc, bytes);\n")
        code += "}\n"
        code += "\n"
        return code
    ...
    def get_encode_code(self):
        ...
        if self.contains_muxed_signals():
            ...
            for mux in muxes:
                ...
                code += ("\n/* Encode " + self.sender + "* " + self.name + " MUX(" + str(mux) + ") message\n")
                code += ("* @returns the message header of this message*/\n")
                code += ("static inline void dbc_encode_" + name_with_mux)
            ...
            name = self.get_struct_name()
            code += ("\n/* Encode " + self.sender + "* " + self.name + " message\n")
            code += ("* @returns the message header of this message */\n")
            code += ("static inline void dbc_encode_" + name[:-2] + "(uint8_t bytes[8], " + name + " *from)\n")
            code += ("{\n")
            code += ("    uint32_t raw;\n")
            code += ("    bytes[0]=bytes[1]=bytes[2]=bytes[3]=bytes[4]=bytes[5]=bytes[6]=bytes[7]=0;\n")
            code += ("\n")
    ...
    def get_non_mux_signal_decode_code(self, raw_sig_name, prefix=''):
        ...
        return code
    def get_signal_decode_print(self, prefix=''):
        code = ''
        for key in self.signals:
            if not self.signals[key].is_muxed():
                code += ("    PRINT(\"" + self.signals[key].name + ":" + self.signals[key].get_code_var_format() + "\\n\", " + prefix + self.signals[key].name + ");\n")
        return code
    ...
    def get_decode_code(self):
        raw_sig_name = "raw"
        code = ''
        code += ("\n/* Decode " + self.sender + "* " + self.name + " message\n")
        code += (
        "* @param hdr  The header of the message to validate its DLC and MID; this can be NULL to skip this check */\n")
        code += ("bool dbc_decode_" + self.get_struct_name()[
                                                    :-2] + "(" + "uint8_t *pstu, const uint8_t *bytes)\n")
        code += ("{\n")
        ...
        #code += ("        return !success;\n")
        code += ("    if (NULL == pstu || NULL == bytes)" + " {\n")
        code += ("        return !success;\n")
        code += ("    }\n\n")
        code += ("    uint32_t " + raw_sig_name + ";\n")
        code += ("    " + self.get_struct_name() + " *to = " + "(" + self.get_struct_name() + " *" + ")" + "pstu" + ";\n")
        code += ("\n")
        ...
        if self.contains_muxed_signals():
            ...
            code += self.get_signal_decode_print("to->")
...
class DBC(object):
    ...
    def gen_msg_hdr_struct(self):
        ...
        code += ("    bool (*func)(uint8_t *pstu, const uint8_t *bytes); //< Data length of the message\n")
        code += ("} dbc_msg_hdr_t; \n")
        return code
    ...
    def gen_msg_extern_func(self):
        code = ''
        code = ("/* CAN message decode extern function */\n")
        for mkey in self.messages:
            m = self.messages[mkey]
            if not self.gen_all and not m.is_recipient_of_at_least_one_sig(
                    self.self_node) and self.self_node != m.sender:
                code += "// "
            code += ("bool " + "dbc_decode_" + m.name + "(uint8_t *pstu, const uint8_t *bytes)" + ";\n")
        code += ("\n")
        return code

    def gen_msg_hdr_instances(self):
        code = ''
        code += ("static const dbc_msg_hdr_t list_dbc_msgs[] = ")
        code += ("{\n")
        for mkey in self.messages:
            m = self.messages[mkey]
            if not self.gen_all and not m.is_recipient_of_at_least_one_sig(
                    self.self_node) and self.self_node != m.sender:
                code += "// "
            code += ("    {" + str(m.mid).rjust(4) + ", " + m.dlc + ", " + ("dbc_decode_" + m.name).rjust(28) + "},\n")
        code += ("};\n")
        return code
...
def main(argv):
    ...
    while 1:
        ...
        if line.startswith(" SG_ "):
            ...
            if (int(bit_start) - 7 + int(bit_size)) > (int(msg_length) * 8):
                print('/// ERROR /')
            ...
            if (float(max_val) != 0) and (float(max_val)) > (float(signal_max)):
                ...
                print('/// ERROR /')
                print('')
                raise ValueError('#error ' + t[1] + ' max value too high. Max value is: ' + str(signal_max) + str(min_val) + str(max_val))
            '''
            unit = t[6].strip('""')
        ...
        if line.startswith("CM_ "):
            ...
            if tp == "SG_":
                ...
                if sig_mid in dbc.messages:
                    if sig_name in dbc.messages[sig_mid].signals:
                        dbc.messages[sig_mid].signals[sig_name].comment = comment.strip(';')
    ...
    h = open("./out/dbc.h", "w")
    h.write(dbc.gen_file_header())
    h.write("\n")
    
    h.write("#ifdef DEBUG\n")
    h.write("#define PRINT printf\n")
    h.write("#else\n")
    h.write("#define PRINT \n")
    h.write("#endif\n")
 
    # Generate the application send extern function
    h.write("/* Extern function needed for dbc_encode_and_send() */\n")
    h.write("extern bool dbc_app_send_can_msg(uint32_t mid, uint8_t dlc, uint8_t bytes[8]);\n")
    h.write("/* Extern function needed for dbc_msg_decode() */\n")
    h.write("extern bool dbc_msg_decode(uint32_t mid, uint8_t *from, uint8_t *to);\n\n")

    h.write(dbc.gen_msg_extern_func())
 
    # Generate header structs and MIA struct
    h.write(dbc.gen_mia_struct())
    h.write(dbc.gen_msg_hdr_struct())
    h.write(dbc.gen_enum_types())
    ...
    for mid in dbc.messages:
        ...
        if not gen_all and not m.is_recipient_of_at_least_one_sig(self_node) and m.sender != self_node:
            ...
            h.write(m.gen_converted_struct(self_node, gen_all))
    ...
    f_mia = open("./out/dbc_handle_mia.c", "w")
    f_mia.write("#include \"dbc.h\" \n")
    ...
    for mid in dbc.messages:
        ...
        if gen_all or m.is_recipient_of_at_least_one_sig(self_node):
            if m.contains_muxed_signals():
                muxes = m.get_muxes()
                for mux in muxes[1:]:
                    f_mia.write(str("extern const uint32_t ").ljust(50) + (m.name + "_" + mux + "__MIA_MS;\n"))
                    f_mia.write(str("extern const " + m.get_struct_name()[:-2] + "_" + mux + "_t").ljust(49) + " " + (
                    m.name + "_" + mux + "__MIA_MSG;\n"))
            else:
                f_mia.write(str("extern const uint32_t ").ljust(50) + (m.name + "__MIA_MS;\n"))
                f_mia.write(str("extern const " + m.get_struct_name()).ljust(49) + " " + (m.name + "__MIA_MSG;\n"))

    # Generate encode methods
    f_en = open("./out/dbc_encode.c", "w")
    f_en.write("#include \"dbc.h\"\n")
    for mid in dbc.messages:
        m = dbc.messages[mid]
        if not gen_all and m.sender != self_node:
            ...
            f_en.write(m.get_encode_code())
    f_en.close()
 
    # Generate decode methods
    f_dec = open("./out/dbc_decode.c", "w")
    f_dec.write("#include \"dbc.h\"\n")
    f_dec.write("\n")
    f_dec.write(dbc.gen_msg_hdr_instances())
    f_dec.write("\n")
    for mid in dbc.messages:
        m = dbc.messages[mid]
        if not gen_all and not m.is_recipient_of_at_least_one_sig(self_node):
            ...
            f_dec.write(m.get_decode_code())
    code = ""
    code += ("bool dbc_msg_decode(uint32_t mid, uint8_t *from, uint8_t *to)\n")
    code += ("{\n")
    code += ("    if (NULL == from || NULL == to)\n")
    code += ("        return false;\n\n")
    code += ("    for (int i = 0; i < sizeof(list_dbc_msgs); i++) {\n")
    code += ("        if (mid == list_dbc_msgs[i].mid)\n")
    code += ("        {\n")
    code += ("            list_dbc_msgs[i].func(to, from);\n")
    code += ("        }\n\n")
    code += ("    }\n") 
    code += ("    return true;\n")
    code += ("}\n")    
